...
def nonLinearBlock(x, g_pre, g_bias, k_p, k_n, g_p, g_n, g_wet, g_post, sos):
    ...
    x_pre = g_pre * x
    x_bias = signal.sosfilt(sos, np.abs(x_pre))
...
def lm_func(t, p_init):
    ...
    y_hat = nonLinearBlock(x, p_init[0], p_init[1], p_init[2], p_init[3], p_init[4], p_init[5], p_init[6], p_init[7], sos)
